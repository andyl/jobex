defmodule JobexWeb.Jobs.Components.HomeBody do

  use Phoenix.Component

  alias JobexWeb.Jobs.Components.Util

  import JobexWeb.Jobs.Components.JobTable
  import Phoenix.HTML

  attr :tick, :integer, required: true

  def job_header(assigns) do
    ~H"""
      <div class="pb-4 flex">
        <div class="pr-5 w-30">BODY TITLE</div>
        <Util.clock tick={@tick} />
      </div>
    """
  end

  attr :body_data, :any, required: true
  attr :uistate, :any, required: true
    ...
    <table>
      <%= for job <- @body_data do %>
        <.tr job_state={job.state}>
          ...
          <.td>TBD<%# tbl_start(job) %></.td>
          <.td><%= raw tbl_secs(@uistate, job) %></.td>
          <.td>
            <%= raw tbl_cmd(@uistate, job) %>
          </.td>
        </.tr>
      <% end %>
    </table>
  ...
  def job_footer(assigns) do
    ~H"""
    """
  end

  # def render(assigns) do
  #   IO.inspect(assigns, label: "BODY ASSIGNS")
  #   ~H"""
  #   <div>
  ...
  def my_live_link(_lbl, _path) do
    """
    <li class="page-item">
    OK
